...
/*   Updated: 2020/09/27 20:02:23 by sreijola         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (((++pc_row + cur_row) < d->my - d->py + 1) && (pc_row < d->py))
	{
		...
		while ((++pc_col + cur_col < d->mx - d->px + 1) && pc_col < d->px)
		{
			if (fit_pc(d, pc_row + cur_row, pc_col + cur_col, valid_move) == 1)
				...
				< d->heat_score)// || (tmp == d->heat_score && tiebreaker(d, 
				// ((pc_row + cur_row) - d->pty), ((pc_col + cur_col) - d->plx))))
				{
					d->heat_score = tmp;
					...
					ft_printf("{#FD}FITS SO HEAT:%d: %d %d\n", d->fd, d->heat_score, pc_row + cur_row, pc_col + cur_col);
					ft_printf("{#FD}%d %d\n", d->fd, d->lmoy, d->lmox);
				}
		}
	}
