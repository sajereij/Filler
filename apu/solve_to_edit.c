...
/*   Updated: 2020/09/20 17:39:28 by sreijola         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_heatmap(t_d *d, int oppo_y, int oppo_x)
{
	...
	while (++cur_row < d->my)
	{
		...
		while (++cur_col < d->mx)
		{
			// distance = ft_abs(cur_row - oppo_y) + ft_abs(cur_col - oppo_x); //manhattan distance: must walk arount the block
...
void	make_heatmap(t_d *d)
{
	...
	max_out_hmap(d); //tämä myös ylempään iffiin?
	map_to_heatmap(d);
}
